...
internal class Program
{
    private static async Task Main(string[] args)
    {
        ...
        var host = Host.CreateDefaultBuilder(args)
            ...
            .ConfigureServices((context, services) =>
            {
                ...
                var hamqthUser = configuration["HamQth:Username"] ??
                                 throw new InvalidOperationException("HamQTH Username missing");
                var hamqthPassword = configuration["HamQth:Password"] ??
                                     throw new InvalidOperationException("HamQTH Password missing");
                var qrzUserName = configuration["QRZ:Username"] ??
                                  throw new InvalidOperationException("QRZ Username missing");
                var qrzPassword = configuration["QRZ:Password"] ??
                                  throw new InvalidOperationException("QRZ Password missing");
        ...
        if (dxccEntity != null) Console.WriteLine($"Found DXCC: {dxccEntity.Name} ({dxccEntity.CountryCode})");
    ...
    {
        var dxc = new DxClusterClient("k4zr.no-ip.org", 7300);
    ...
    private static async Task RigTest()
    {
        var client = new HamLibRigCtlClient("localhost", 4532);
